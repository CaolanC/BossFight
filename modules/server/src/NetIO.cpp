...
namespace server
{
    ...
    void NetIO::setup_routes() {
        ...
        // POST join_session/{session_id}/join_as_god
        router.GET("join_as_god", [this](HttpRequest* request, HttpResponse* response) {
            std::promise<JoinAsGodReplyPayload> promise;
            auto fut = promise.get_future();
            auto id = xg::newGuid();
            {
                std::lock_guard<std::mutex> lock(req_prom_mut_join);
